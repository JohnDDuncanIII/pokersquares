...
public class Card implements Comparable<Card>, Serializable {
    ...
    public static boolean isRoyalFlush(List<Card> ar) {
		...
		if(isFlush(ar) && val1.name()=="TEN" && val2.name()=="JACK" && val3.name()=="QUEEN" && val4.name()=="KING" && val5.name() == "ACE") {
			return true;
		}
		return false;
	}
	...
	public static boolean isFullHouse(List<Card> ar) {
		...
		if(((val1 == val2 && val2 == val3) && val4 == val5) || (val1 == val2 && (val3 == val4 && val4 == val5))) {
			return true;
		}
		return false;
	}
	...
	public static boolean hasTwoPair(List<Card> ar) {
		for (int i = 0; i < ar.size()-3; i++) {
			Card.Rank firstValue  = ar.get(i).rank();
			Card.Rank secondValue = ar.get(i+1).rank();
			Card.Rank thirdValue  = ar.get(i+2).rank();
			Card.Rank fourthValue  = ar.get(i+3).rank();
